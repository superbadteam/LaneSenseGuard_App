...
<template>
  ...
  <div v-if="user"
    class="overflow-hidden rounded-xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    ...
    <div class="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
      <div
        ...
        <div class="relative drop-shadow-2">
          <img v-image="user.avatar" alt="profile" />
          <!-- <label
              for="profile"
      ...
      <div class="mt-4">
        <h3 class="mb-1.5 flex justify-center items-center gap-2 text-2xl font-medium text-black text-center dark:text-white">
          {{ user.name }}
          <div class="bg-white w-fit -left-3 rounded-full  -bottom-2 ">
                      <div class="flex items-center bg-opacity-10 rounded-full  py-1 px-2 text-xs font-medium" :class="{
                      'bg-danger text-danger': user.membership === 2,
                      'bg-primary text-primary': user.membership === 1
                      }">
                        {{ user.membership === 1 ? 'Standard' : 'Premium' }}
                      <ISvg icon="fluent:premium-20-filled" v-if="user.membership === 2"/>
                    </div>
                  </div>
        </h3>
        <p class="font-medium">{{ user.email }}</p>
        <div
          class="mx-auto mt-4.5 mb-5.5 grid  grid-cols-4 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F]">
          <div
          ...
          <div
            class="flex flex-col items-center justify-center gap-1 border-r border-stroke px-4 dark:border-strokedark xsm:flex-row">
            <span class="font-semibold text-black dark:text-white">Gender:</span>
            <span class="text-sm">{{ user.gender ? 'Male' : 'Female' }}</span>
          </div>
          <div
            class="flex flex-col items-center justify-center gap-1 border-r border-stroke px-4 dark:border-strokedark xsm:flex-row">
            <span class="font-semibold text-black dark:text-white">Birthday:</span>
            <span class="text-sm">{{ user.date_of_birth.split("T")[0] }}</span>
          </div>
          <div class="flex flex-col items-center justify-center gap-1 px-4 xsm:flex-row">
            ...
            <p class="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium" :class="{
              'bg-warning text-warning': user.role === 2,
              'bg-success text-success': user.role === 1
            }">
