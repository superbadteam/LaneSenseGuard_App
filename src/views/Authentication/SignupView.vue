...
const onSubmit = async () => {
  if (password.value !== retypePassword.value) {
    notify('Password does not match', {type: 'error'})
    return
  }
  try {
  console.log('Email:', email.value)
  console.log('Password', password.value)
  const data = await signUpApi(email.value, password.value, name.value, gender.value, birthdate.value);
  console.log(data);  
...
<template>
    ...
    <DefaultAuthCard subtitle="Start for free" title="Sign Up to Safesurf" class="h-fit">
      <form @submit.prevent="onSubmit">
        <InputGroup label="Name" type="text" placeholder="Enter your name" v-model="name">
          <svg
            class="fill-current"
            width="22"
            height="22"
        ...
        <InputGroup label="Re-type Password" type="password" placeholder="Re-enter your password" v-model="retypePassword">
          <svg
            class="fill-current"
            width="22"
            height="22"
