...
onBeforeMount(() => {
    getCategories().then((res) => {
        console.log(res)
        options.value = res.map((option: any) => ({
            value: option._id,
            text: option.name,
            selected: false
        }))
        ...
        <div class="relative z-20 inline-block w-full">
          <div class="relative flex flex-col items-center">
            <div @click="open" class="w-full">
              <div
                class="flex rounded-md border border-stroke py-2 pl-3 pr-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input"
              >
